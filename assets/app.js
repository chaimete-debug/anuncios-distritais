...
// Indicador de carregamento para diagnóstico.
window.__ANUNCIOS_APP_LOADED__ = true;
...
window.addEventListener('error', event => {
  console.error('Erro da aplicação:', event.error || event.message);
  showClientError('A aplicação encontrou um erro ao iniciar. Actualize a página com Ctrl + F5.');
});
...
const PWA_INSTALLED_KEY = 'districtAnnouncementsPwaInstalled';
const PWA_SKIP_LANDING_KEY = 'districtAnnouncementsSkipInstallLanding';
...
function updateInstallView() {
  ...
  installStatus.hidden = !installed;
}
...
async function launchApplication() {
  sessionStorage.setItem(PWA_SKIP_LANDING_KEY, 'true');
...
async function api(action, payload = {}) {
  ...
  try { result = JSON.parse(responseText); }
    ...
    const hint = response.status === 404
      ? 'O ficheiro api/gas.js não foi encontrado no Vercel.'
...
$('installAppBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('Não foi possível abrir a instalação:', err);
  }
});

$('openAppBtn').addEventListener('click', launchApplication);
...
async function init() {
  ...
  const bypassLanding = isStandaloneMode() ||
    sessionStorage.getItem(PWA_SKIP_LANDING_KEY) === 'true';
